...
\end{tabular}
\caption{Summary of the $30$ different velocity divergence fields used in this Letter. 
The Field type describes whether the field comes from the Abacus Simulations (N-body) or it is generated from the power spectrum (Gaussian). $v_max$ only applies to the N-body fields and refers to the minimum value in the haloes' maximum circular velocity to be included in the peculiar velocity field interpolation.
$L_b$ is the box size of the cubic volume used to compute the divergence. $R_{s}$ is the list of lengths describing the gaussian smoothing (in the case of N-body fields) and the wavelentgh $1/R_s$ used to truncate the power spectrum (in the case of Gaussian fields). $N_c$ is the number of grid cells on a side of the interpolated density field.}
\label{table:values}
\end{table*}
...
We use a total of $30$ different divergence fields in this Letter. 
...
Table \ref{table:values} summarizes the different values.
...
We use a watershed algorithm \citep{BeucherWatershed1979} on the velocity divergence field to find superclusters.
...
We describe the supercluster population in a divergence grid by the average value of the equivalent radius, $\langle R_{\rm eq}\rangle$.
...
\begin{figure*}
    ...
    In all cases the relationship between the two quantities is close to linear for auto-correlation lenghts below $100$ \Mpch. However, with the parameterization $\langle R_{\rm{eq}}\rangle=\alpha R_{\delta\delta}$, the best fit gives $\alpha\approx1.25$ for Gaussian fields and $\alpha\approx0.9$ for N-body fields.
    \label{fig:Nclusters}}
\end{figure*}
