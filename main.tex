...
\maketitle
\begin{abstract}
Superclusters are emerging as a new way to spatially partition the Universe, both in observations and simulations.
Different definitions and algorithms have been presented in the literature with the expectation to build an stable framework to use superclusters in a quantitative fashion.
In this Letter we show the strengths of defining superclusters as watershed basins in the divergence velocity field.
We apply this definition on data generated from linear theory and data from N-body simulations and experiment with different grid sizes, smoothing scales and types of tracers.
Under this perspective we find that the average supercluster size linearly follows the autocorrelation length in the divergence field, a result that holds for one order of magnitude from 10 \Mpch up to 100 \Mpch.
Our results suggest that the divergence-based definition provides a robust context to quantitatively compare results across different observational or computational frameworks. Its flexibility can also facilitate a better understanding of how supercluster properties depend on cosmological parameters and explore the possibility of using superclusters as comological probes.
\end{abstract}
...
They interpolate the dark matter density field on a grid, smooth it with a gaussian fielters and then define different density cuts above which superclusters are defined and characterized. 
...
The divergence not only help us to keep a quantitative understanding of how the supercluster properties depend on the smoothing scale used build the velocity field, but also provides a handle to tie the superclusters to the underlying cosmology.
