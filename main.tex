...
For instance, measuring to what extent the linearity between scales  in Figure \ref{fig:correlation length} depends on the box size, or comparing supercluster sizes as computed from different definitions that take the similar data as an input.
As an application of our results we would give priority to studying scaling relationships between sizes, 3D shapes, maximum divergence depth, and studying (via linear theory) the dependence of supercluster properties on cosmological parameters and its redshift evolution.
