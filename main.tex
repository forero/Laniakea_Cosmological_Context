...
Super-clusters in velocity fields are usually defined as regions of converging peculiar velocity flows.
The spatial scale over which the velocity field is coherent thus defines the properties of these converging flow basins.
Larger coherence length translates into larger super-clusters.

In this Letter we presented one possible way to unify and quantify this picture.
Starting from the divergence of the velocity field, we compute its auto-correlation function to give a precise definition of the coherence scale as an auto-correlation length. 
On the other hand watershed partitioning over the velocity divergence field produces the super-clusters, from which we can compute an average size.

We tested this picture using very different divergence fields (Gaussian and non-linear from N-body simulations) to find that the typical super-clusters size linearly follows the auto-correlation length.
This results holds for one order of magnitude from $10$\Mpch up to $100$\Mpch.

Future tasks that can be performed using the framework presented here include: characterizing the super-clusters shapes, find explicit dependence of super-cluster properties on the cosmological parameters and finding relationships between the divergence based superclusters and segmentation based on graphs defined on the galaxy spatial distribution.
