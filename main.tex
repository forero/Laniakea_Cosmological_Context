...
\maketitle
\begin{abstract}
Galaxy superclusters are useful way to partition the Universe in a way that can be probed both by observations and simulations.
In this Letter explore the robustness of defining  superclusters as watershed basins in the divergence velocity field.
To characterize this field with a global scalar we also define an auto-correlation length from the autocorrelation function.
We use divergence fields computed both from linear theory and direct interpolation from N-body simulations to apply these definitions and find that the average supercluster size linearly follows the autocorrelation length. 
This result holds for one order of magnitude from 10 \Mpch up to 100 \Mpch.
We estimate that, if the Laniakea supercluster is to be considered to have an average size then the auto-correlation length in the divergence fields from the Cosmicflows dta data should be on the order of 65 \Mpch to 90  \Mpch.
This supercluster definition naturally allows the study of supercluster abundance properties as a function of cosmological parameters and redshift.
\end{abstract}
...
Different perspectives have been used to define superclusters. 
...
classification of the cosmic web is applied to cosmological N-body simulations.  
...
flows.
This conceptual statement was been recently exploited by \cite{Dupuy_2019} 
...
The properties of the basins of attraction also depend on two more free parameters: the streamline length and the number of streamlines ending on a vortex required to define a gravitational basing.
Additionally, the
...
In this Letter we suggest an alternative path to have a robust identification of superclusters from velocity data with the lowest possible number of free parameters for the supercluster finding algorithm, while keeping a clear quantitative understanding of how the supercluster properties depend on the smoothing scale used build the velocity field.
